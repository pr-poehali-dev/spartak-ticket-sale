...
function StadiumMap(props: StadiumMapProps) {
  ...
  return (
    <svg viewBox={`0 0 ${SW} ${SH}`} className="w-full"
      ...
      {/* Platinum — единый блок A4+A5 (верх) */}
      <SectorRect id="APT" x={326} y={40} w={108} h={32} {...props} fontSize={7} />
      {/* A4 Gold (низ) + A5 Silver (низ) */}
      ...
      {[['C115',252,536],['C114',304,536],['C113',356,536],
        ['C112',408,536],['C111',460,536]
      ].map(([id, x, y]) => (
        <SectorRect key={id as string} id={id as string} x={x as number} y={y as number} w={48} h={38} {...props} fontSize={6.5} />
      ))}
      {/* C116 — рядом с C115 */}
      <SectorRect id="C116" x={200} y={536} w={48} h={38} {...props} fontSize={6.5} />
      {/* C110 — поднят к C111 */}
      <SectorRect id="C110" x={512} y={536} w={48} h={38} {...props} fontSize={6.5} />
      {[['C109',210,578]].map(([id, x, y]) => (
      ...
      {[['C220',220,578],
        ['C219',266,578],['C218',312,578],['C217',358,578],['C216',404,578],
        ['C215',450,578],['C214',496,578],['C213',542,578],['C212',588,578],['C211',634,578]
      ].map(([id, x, y]) => (
